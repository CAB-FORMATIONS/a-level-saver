...
from src.constants.departments import DEPT_DOC, DEPT_DOC_ID, DEPT_CONTACT, DEPT_DOCS_CAB, DEPT_REFUS_CMA, DEPT_COMPTABILITE
from src.constants.emails import AUTO_CLOSE_SENDERS, COMPANY_SIGNATURE
...
class DOCTicketWorkflow:
    ...
    # Disabled after the July 2026 production audit. A scenario may only be
    # restored with an exact subject, intent and state allowlist.
    AUTO_SEND_SCENARIOS = []

    @staticmethod
    def _normalize_sender_email(value: str) -> str:
        """Extract a lowercase email from Zoho display-name formats."""
        parsed = parseaddr(value or '')[1]
        return (parsed or value or '').strip().strip('<>').lower()

    @staticmethod
    def _normalize_auto_send_subject(value: str) -> str:
        subject = (value or '').lower().strip()
        subject = re.sub(r'^(?:(?:re|tr|fw|fwd)\s*:\s*)+', '', subject)
        return re.sub(r'\s+', ' ', subject)

    def _ticket_context_is_current(self, ticket_id: str, snapshot: Dict) -> bool:
        """Check that ticket ownership and conversation stayed unchanged."""
        expected_incoming_id = str(snapshot.get('latest_incoming_thread_id') or '')
        expected_thread_id = str(snapshot.get('latest_thread_id') or '')
        expected_status = str(snapshot.get('ticket_status_snapshot') or '').lower()
        expected_department = str(snapshot.get('department_snapshot') or '')
        if not expected_incoming_id or not expected_thread_id or not expected_status or not expected_department:
            return False
        try:
            ticket = self.desk_client.get_ticket(ticket_id)
            current_status = str(ticket.get('statusType') or ticket.get('status') or '').lower()
            current_department = str(ticket.get('departmentId') or ticket.get('department', {}).get('name') or '')
            if current_status != expected_status or current_department != expected_department:
                return False

            response = self.desk_client.get_ticket_threads(ticket_id)
            threads = response.get('data', []) if isinstance(response, dict) else response
            ordered_threads = [thread for thread in threads if isinstance(thread, dict)]
            ordered_threads.sort(key=lambda thread: thread.get('createdTime') or '', reverse=True)
            latest_thread_id = str(ordered_threads[0].get('id') or '') if ordered_threads else ''
            incoming = [
                thread for thread in threads
                if isinstance(thread, dict)
                and thread.get('direction') == 'in'
                and str(thread.get('status', '')).upper() != 'DRAFT'
            ]
            incoming.sort(key=lambda thread: thread.get('createdTime') or '', reverse=True)
            latest_incoming_id = str(incoming[0].get('id') or '') if incoming else ''
            return latest_incoming_id == expected_incoming_id and latest_thread_id == expected_thread_id
        except Exception as exc:
            logger.warning(f"Impossible de revalider le contexte du ticket {ticket_id}: {exc}")
            return False

    def _refresh_ticket_ownership_snapshot(self, ticket_id: str, snapshot: Dict) -> None:
        """Accept an intentional department transfer without hiding other changes."""
        ticket = self.desk_client.get_ticket(ticket_id)
        department = ticket.get('department') or {}
        snapshot['department_snapshot'] = str(
            ticket.get('departmentId')
            or (department.get('name') if isinstance(department, dict) else '')
            or ''
        )
    ...
    def _can_auto_send(self, response_result: Dict, triage_result: Dict = None) -> tuple:
        ...
        Auto-send is deny-by-default. Every enabled scenario must constrain the
        exact campaign subject, primary intent and detected state.
        ...
        subject = self._normalize_auto_send_subject(triage_result.get('ticket_subject') or '')
        incoming_count = triage_result.get('incoming_thread_count', 0)
        intent = triage_result.get('detected_intent') or triage_result.get('primary_intent')
        state_id = (response_result.get('state_engine') or {}).get('state_id')

        subject_scenarios = [
            scenario for scenario in self.AUTO_SEND_SCENARIOS
            if self._normalize_auto_send_subject(scenario.get('subject_equals', '')) == subject
        ]
        if not subject_scenarios:
            return False, f'scenario_not_eligible: {subject[:50]}'

        intent_scenarios = [
            scenario for scenario in subject_scenarios
            if intent == scenario.get('intent_equals')
        ]
        if not intent_scenarios:
            return False, f'intent_not_eligible: {intent}'

        matched_scenario = next((
            scenario for scenario in intent_scenarios
            if state_id == scenario.get('state_equals')
        ), None)
        if not matched_scenario:
            return False, f'state_not_eligible: {state_id}'

        if incoming_count != 1:
            return False, f'incoming_count_not_one: {incoming_count}'

        customer_message = (triage_result.get('customer_message') or '').strip()
        if len(customer_message) < 20:
            return False, 'empty_or_ambiguous_message'

        secondary_intents = response_result.get('secondary_intents') or triage_result.get('secondary_intents') or []
        if secondary_intents:
            return False, 'secondary_intents_present'

        if triage_result.get('method') in {'default', 'fallback', 'error_fallback'}:
            return False, f"unsafe_triage_method: {triage_result.get('method')}"
        if float(triage_result.get('confidence') or 0) < 0.9:
            return False, 'low_triage_confidence'
        if triage_result.get('has_attachments'):
            return False, 'attachments_present'
        if triage_result.get('human_intervention'):
            return False, 'human_intervention_present'
        if triage_result.get('quoted_content_fallback'):
            return False, 'quoted_content_only'
        if triage_result.get('recipient_is_ticket_contact') is not True:
            return False, 'recipient_not_verified'
        if triage_result.get('delivery_context_is_current') is not True:
            return False, 'stale_or_unverified_context'

        # --- Response quality ---
        response_text = response_result.get('response_text', '').strip()
        if not response_text:
            return False, 'empty_response'
        if not response_result.get('was_humanized', False):
            return False, 'humanizer_failed'
        validations = response_result.get('validation') or {}
        if not validations:
            return False, 'validation_missing'
        for val in validations.values():
            if not val.get('compliant', False) or val.get('errors'):
                return False, 'validation_errors'
            if val.get('warnings'):
                return False, 'validation_warnings'

        if response_result.get('is_blocking'):
            return False, 'blocking_state'

        plain_response = re.sub(r'<[^>]+>', ' ', response_text)
        if len(re.findall(r'\b\w+\b', plain_response)) > 140:
        ...
        if any(re.search(pattern, plain_response, re.IGNORECASE) for pattern in sensitive_patterns):
            return False, 'sensitive_data_present'
    ...
    def process_ticket(
        ...
        try:
            ...
            ticket_status = str(ticket_data.get('statusType') or ticket_data.get('status') or '').lower()
            if ticket_status == 'closed':
                logger.info("Ticket déjà fermé → aucun traitement ni brouillon")
                result['workflow_stage'] = 'SKIPPED_TICKET_CLOSED'
                result['success'] = True
                result['skip_reason'] = 'Ticket déjà fermé'
                return result
            ...
            triage_result['human_intervention'] = bool(agent_hint)
            if triage_result.get('transferred'):
                self._refresh_ticket_ownership_snapshot(ticket_id, triage_result)
            ...
            # Terminal sender/content filters must run before duplicate or
            # identity clarification logic, which may write to Desk or CRM.
            terminal_action = triage_result.get('action')
            if terminal_action == 'SPAM':
            ...
            if terminal_action == 'CMA_NOTIFICATION':
                ...
                result['success'] = True
                return result
            if terminal_action == 'CMA_OTHER':
                logger.warning("🏛️ Email CMA non catégorisé → aucun brouillon client")
                result['workflow_stage'] = 'SKIPPED_CMA_OTHER'
                result['success'] = True
                return result
            if terminal_action == 'NOREPLY_NOTIFICATION':
            ...
            writes_enabled = auto_create_draft or auto_update_crm or auto_update_ticket or auto_send
            if writes_enabled and not self._ticket_context_is_current(ticket_id, triage_result):
                logger.warning("Traitement annulé: nouveau message reçu après le triage")
                result['workflow_stage'] = 'SKIPPED_STALE_CONTEXT_AFTER_TRIAGE'
                result['success'] = True
                result['send_fallback_reason'] = 'stale_or_unverified_context'
                return result
...
⚠️ NE PAS REPAYER en ligne les {CMA_EXAM_FEE}€ de frais d'examen."""
            ...
            if writes_enabled and not self._ticket_context_is_current(ticket_id, triage_result):
                logger.warning("Mises à jour annulées: nouveau message reçu pendant l'analyse")
                result['workflow_stage'] = 'SKIPPED_STALE_CONTEXT_BEFORE_UPDATES'
                result['success'] = True
                result['send_fallback_reason'] = 'stale_or_unverified_context'
                return result
            ...
            if auto_send or auto_create_draft:
                ...
                try:
                    ...
                    ticket_status = str(ticket.get('statusType') or ticket.get('status') or '').lower()
                    if ticket_status == 'closed':
                        logger.warning("Reply delivery annulé: ticket déjà fermé")
                        result['workflow_stage'] = 'SKIPPED_TICKET_CLOSED_BEFORE_DELIVERY'
                        result['success'] = True
                        result['send_fallback_reason'] = 'ticket_closed_before_delivery'
                        return result
                    if auto_send and department not in {DEPT_DOC, DEPT_DOC_ID}:
                        logger.warning(f"Auto-send annulé: ticket déplacé vers {department}")
                        result['success'] = True
                        result['send_fallback_reason'] = 'ticket_left_doc_department'
                        return result
                    ...
                    triage_result['recipient_is_ticket_contact'] = bool(
                        self._normalize_sender_email(to_email or '')
                        and self._normalize_sender_email(to_email or '')
                        == self._normalize_sender_email(ticket.get('email') or '')
                    )
                    ...
                    triage_result['delivery_context_is_current'] = self._ticket_context_is_current(
                        ticket_id,
                        triage_result,
                    )
                    if not triage_result['delivery_context_is_current']:
                        logger.warning("Livraison annulée: nouveau message ou contexte non vérifiable")
                        result['success'] = True
                        result['send_fallback_reason'] = 'stale_or_unverified_context'
                        return result
    ...
    def _run_triage(self, ticket_id: str, auto_transfer: bool = True, agent_hint: Optional[str] = None) -> Dict:
        ...
        incoming_thread_count = sum(
            1 for t in threads
            if isinstance(t, dict)
            and t.get('direction') == 'in'
            and str(t.get('status', '')).upper() != 'DRAFT'
        )
        incoming_threads = [
            t for t in threads
            if isinstance(t, dict)
        ...
        incoming_threads.sort(key=lambda t: t.get('createdTime') or '', reverse=True)
        latest_incoming = incoming_threads[0] if incoming_threads else {}
        all_threads = [t for t in threads if isinstance(t, dict)]
        all_threads.sort(key=lambda t: t.get('createdTime') or '', reverse=True)
        latest_thread = all_threads[0] if all_threads else {}
        try:
            attachment_count = int(latest_incoming.get('attachmentCount') or 0)
        except (TypeError, ValueError):
            attachment_count = 0
        latest_has_attachments = bool(
            latest_incoming.get('hasAttach')
            or attachment_count > 0
            or latest_incoming.get('attachments')
        )
        ...
        triage_result = {
            ...
            'latest_incoming_thread_id': str(latest_incoming.get('id') or ''),
            'latest_thread_id': str(latest_thread.get('id') or ''),
            'ticket_status_snapshot': str(ticket.get('statusType') or ticket.get('status') or '').lower(),
            'department_snapshot': str(current_department),
            'has_attachments': latest_has_attachments,
            'quoted_content_fallback': quoted_content_fallback,
        }
        ...
        for _th in threads:
            if _th.get('direction') == 'in' and _th.get('status') != 'DRAFT':
                author = _th.get('author') or {}
                raw_sender = _th.get('fromEmailAddress') or author.get('email') or ''
                most_recent_from = self._normalize_sender_email(raw_sender)
                break
        ...
        if most_recent_from in AUTO_CLOSE_SENDERS:
            source_name = AUTO_CLOSE_SENDERS[most_recent_from]
